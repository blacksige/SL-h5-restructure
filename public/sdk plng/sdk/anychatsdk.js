...
var BRAC_SO_H5WEBSDK_VERSION                = 1000; //H5SDK版本设置（1 适配器版本（默认）, 2 原始版本）,在调用BRAC_Connect接口前设置
var BRAC_SO_H5WEBSDK_BLOBDATA               = 1001; //获取转换blob数据块
var BRAC_SO_H5WEBSDK_STUNSERVICEADDR		= 1003; //设置stun服务器的地址（包含端口），参数为字符串值
var BRAC_SO_H5WEBSDK_WEAKNETWORKMODE		= 1004; //设置弱网下的工作模式（参数为int型，0 浏览器默认[默认]， 1 降低帧率保分辨率处理）
...
var BRAC_STREAMINFO_AUDIOFLAG               = 501; //音频媒体标志(请求音视频前调用， 1:开启音频 0:不开启音频)
var BRAC_STREAMINFO_VIDEOFLAG               = 502; //视频媒体标志(请求音视频前调用， 1:开启视频 0:不开启视频)
...
// 传输任务信息参数定义（API：BRAC_TransFileEx 传入参数）
var ANYCHAT_TRANS_FILE_DATA_TYPE_BASE4		= 1;	//base64数据
var ANYCHAT_TRANS_FILE_DATA_TYPE_BINARY		= 2;	//二进制数据
...
var BRAC_RECORD_FLAGS_LOCALCB               = 0x00000800; // 触发本地回调BRAC_SnapShot
...
var WM_GV_ICELINKCLOSE                      = WM_GV + 500; // 用户网络异常，音视频链接建立失败，wParam（INT）表示用户ID号, lParam（INT）表示连接结果：0 成功，-1 失败
var WM_GV_MEDIAAUTHORITY                    = WM_GV + 501; // 获取媒体权限结果，wParam（INT）表示用户成功与否，0 成功, -1 失败， lParam（string） 表示错误信息
var WM_GV_H5MEDIAOPENSTATUS                 = WM_GV + 502; // H5媒体状态，wParam（INT）表示媒体开启状态成功与否，0 成功, -1 失败， lParam（string） 表示错误信息
var WM_GV_QUERYINFOFORMSERVER				= WM_GV + 503; // 服务器信息查询,wParam 表示服务器返回的查询结果，lParam 为出错代码：0 成功
var WM_GV_STREAMRECORDCTRLEX				= WM_GV + 504; // BRAC_StreamRecordCtrlEx接口执行结果，lParam 为出错代码：0 成功
var WM_GV_SETSDKOPTION						= WM_GV + 505; // BRAC_SetSDKOption接口执行结果，lParam 为出错代码：0 成功
var WM_GV_SDKCONTROL						= WM_GV + 506; // BRAC_SDKCONTROL接口执行结果，lParam 为出错代码：0 成功
var WM_GV_USERCAMERACONTROL					= WM_GV + 507; // BRAC_UserCameraControl、BRAC_UserCameraControlEx接口执行结果，lParam 为出错代码：0 成功
var WM_GV_USERSPEAKCONTROL					= WM_GV + 508; // BRAC_UserSpeakControl、BRAC_UserSpeakControlEx接口执行结果，lParam 为出错代码：0 成功
var WM_GV_VIDEOCALLCONTROL					= WM_GV + 509; // BRAC_VideoCallControl接口执行结果，lParam 为出错代码：0 成功
var WM_GV_OBJECTCONTROL						= WM_GV + 510; // BRAC_ObjectControl接口执行结果，lParam 为出错代码：0 成功
var WM_GV_TRANSBUFFER						= WM_GV + 511; // BRAC_TransBuffer、BRAC_TransBufferEx接口执行结果，lParam 为出错代码：0 成功
var WM_GV_OBJECTSETVALUE					= WM_GV + 512; // BRAC_ObjectSetValue接口执行结果，lParam 为出错代码：0 成功
var WM_GV_SENDTEXTMESSAGE					= WM_GV + 513; // BRAC_SendTextMessage接口执行结果，lParam 为出错代码：0 成功
var WM_GV_TRANSFILE							= WM_GV + 514; // BRAC_TransFileEx接口执行结果，lParam 为出错代码：0 成功
var WM_GV_CANCELTRANSTASK					= WM_GV + 515; // BRAC_CancelTransTaskEx接口执行结果，lParam 为出错代码：0 成功
...
var GV_ERR_BROWSERNOWEBRTC                  = 1020000; // 不支持webrtc
...
var ANYCHAT_SDKCTRL_MEDIAAUTHORITY	        = 350; // 媒体权限
// 媒体播放事件类型定义
...
// AI机器人
var ANYCHAT_CORESDKEVENT_AIABILITY          = 102; 
...
// AI能力状态
var ANYCHAT_AIABILITY_STATUS_UNKNOW		    = 0; // 未知状态
...
// AI能力类型定义
var ANYCHAT_AI_TYPE_FEA					    = 102; // 微表情分析（face emotion analysis）

// AI能力厂商
var  AIABILITY_FIRM_PINGAN				    = 4; // 平安一账通 微表情

// 微表情分析工作模式
var ANYCHAT_FEAMODE_FEA = 1;	// 微表情分析

// 微表情分析 ctrlcode
var ANYCHAT_FEA_CTRLCODE_ANALYSIS           = 1; // 微表情分析(开始/结束)
var ANYCHAT_FEA_CTRLCODE_DETECT             = 2; // 微表情检测结果
var ANYCHAT_FEA_CTRLCODE_REQUESTEX          = 3; // 扩展请求（面审建议）
var ANYCHAT_FEA_CTRLCODE_PROCESSDATA        = 4; // 微表情实时分析数据
var ANYCHAT_FEA_CTRLCODE_PAUSE              = 5; // 微表情分析暂停
...
var bSupportH5Media = false;// 是否支持H5媒体

// 初始化SDK，返回出错代码

function BRAC_InitSDK(apilevel) {
    //是否支持Html5	applicationCache API在PC谷歌85版本已废弃，新增Web Workers API判断是否支持Html5
    var isSurportH5 = window.applicationCache;
    var browser = getBrowser();
    //browser == "Safari" ||
    if ((typeof(Worker) !== "undefined" || isSurportH5) && !(browser == "IE")) {
        anychat = null;
        anychat = AnyChatSDK();
    
        if(!anychat){
            return GV_ERR_BROWSERNOWEBRTC;
        }
        anychat.InitSDK(0);
        ...
        if (typeof(OnAnyChatRecordSnapShotEx) == "function")
            anychat.on('OnRecordSnapShotEx', OnAnyChatRecordSnapShotEx);
        if (typeof(OnAnyChatRecordSnapShotEx2) == "function")
            anychat.on('OnRecordSnapShotEx2', OnAnyChatRecordSnapShotEx2);
        if (typeof(OnAnyChatRecordSnapShot) == "function")
            anychat.on('OnRecordSnapShot', OnAnyChatRecordSnapShot);
        if (typeof(OnAnyChatTransBufferEx) == 'function')
            anychat.on('OnTransBufferex',OnAnyChatTransBufferEx);
        if (typeof(OnAnyChatCoreSDKEvent) == "function")
            anychat.on('OnAnyChatCoreSDKEvent', OnAnyChatCoreSDKEvent);
        if (typeof(OnAnyChatTransFileEx) == "function")
            anychat.on('OnAnyChatTransFileEx', OnAnyChatTransFileEx);
		if (typeof(OnAnyChatLinkCloseInstant) == "function")
            anychat.on('OnAnyChatLinkCloseInstant', OnAnyChatLinkCloseInstant);
		if (typeof(OnAnyChatRelink) == "function")
            anychat.on('OnAnyChatRelink', OnAnyChatRelink);
		if (typeof(OnAnyChatGetSDKOptionInt) == "function")
			anychat.on('OnAnyChatGetSDKOptionInt', OnAnyChatGetSDKOptionInt);
		if (typeof(OnAnyChatGetSDKOptionString) == "function")
			anychat.on('OnAnyChatGetSDKOptionString', OnAnyChatGetSDKOptionString);
		if (typeof(OnAnyChatQueryUserState) == "function")
			anychat.on('OnAnyChatQueryUserState', OnAnyChatQueryUserState);
		if (typeof(OnAnyChatObjectGetValue) == "function")
			anychat.on('OnAnyChatObjectGetValue', OnAnyChatObjectGetValue);
		if (typeof(OnAnyChatObjectGetIdList) == "function")
			anychat.on('OnAnyChatObjectGetIdList', OnAnyChatObjectGetIdList);
        bSupportStreamRecordCtrlEx = true;
        bSupportScriptObject = true;
        bSupportObjectBusiness = true;
        bSupportCluster = true;
        bSupportH5Media = true;
    ...
    try {
        ...
        if (bRightVersion) {
            ...
            if (typeof(OnAnyChatTransFileEx) == "function")
                BRAC_RegisterCallBack(anychat, 'OnAnyChatTransFileEx', OnAnyChatTransFileEx);
        } else {
...
function BRAC_SetVideoPosEx(userid, parentobj, id, streamindex) {
	if (bSupportScriptObject)
        return anychat.SetVideoPosEx(userid, parentobj, id, streamindex);
    else
...
function BRAC_UserCameraControlEx(dwUserId, bOpen, dwStreamIndex, dwFlags, szStrParam) {
    if (bSupportH5Media || bSupportMultiStream)
...
// 获取媒体控制权限
function BRAC_GetMediaControlAuthority()
{
    if (bSupportH5Media)
        return anychat.GetMediaControlAuthority();
    else
        return -1;
}

// 基于苹果内核BUG,提供接口可用于修正fasari浏览器播放TTS声音小的问题
function BRAC_SafariVolumeIncrease(){
	if (bSupportH5Media)
        return anychat.safariVolumeIncrease();
    else
        return -1;
}
...
function getBrowser() {
    ...
    var info = {
        ...
        ie11: /trident/.test(ua) && /rv:11.0/.test(ua),
    ...
    if (info.ch) {
        browser = "Chrome";
    } else if (info.ie || info.ie11 || (!!window.ActiveXObject || "ActiveXObject" in window)) {
        browser = "IE";
    } else if (info.ff) {
        browser = "Firefox";
    } else if (info.sa) {
